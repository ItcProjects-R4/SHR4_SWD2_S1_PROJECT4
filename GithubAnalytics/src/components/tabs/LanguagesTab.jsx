...
export default function LanguagesTab({ data = [] }) {
  ...
  return (
    <div
      className={`grid gap-12 ${dualMode ? "grid-cols-1 lg:grid-cols-2" : "grid-cols-1"}`}
    >
      {activeData.map((d, i) => {
              ...
              <div className="flex flex-col gap-4 flex-1 max-w-[280px]">
                {d.langs?.map((lang, li) => {
                  ...
                  return (
                    <div
                      key={li}
                      className="flex items-center justify-between text-[12px] border-b border-zinc-900/50 pb-2"
                    >
                      ...
                      <GlowText size="16px" color={color}>
                        {lang.value}
                      </GlowText>
                    </div>
                  );
